<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $chief->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $chief->id)]
            )
        ?></li>
...
<div class="chiefs form large-9 medium-8 columns content">
    <?= $this->Form->create($chief) ?>
    <fieldset>
        <legend><?= __('Edit Chief') ?></legend>
        <?php
